...
uint8_t NextionSetXVal(Nextion *nex, NexComp *comp, float userXval)
{
	//Allocate a static buffer for combining the transfer command string
	char transmitBuff[NEXTION_TEXT_BUFF_LEN] = {0};

	//Combine required commands in a single string
	sprintf(transmitBuff, "%s.Xval=%f", comp->objname, userXval);
...
uint8_t NextionSetChar(Nextion *nex, NexComp *comp, char charVal)
{
	//Allocate a static buffer for combining the transfer command string
	char transmitBuff[NEXTION_TEXT_BUFF_LEN] = {0};

	//Combine required commands in a single string
	sprintf(transmitBuff, "add 1,0,%c", charVal);
